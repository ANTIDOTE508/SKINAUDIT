...
function wizardReducer(state: WizardState, action: WizardAction): WizardState {
  switch (action.type) {
    case 'SET_STEP':
      return { ...state, step: action.step, interstitialId: null }
    case 'SHOW_INTERSTITIAL':
      return { ...state, interstitialId: action.id }
    case 'CLEAR_INTERSTITIAL':
      return { ...state, interstitialId: null }
    case 'SET_TRANSITIONING':
      return { ...state, isTransitioning: action.value }
    case 'SET_GENDER':
      return { ...state, genderIdentity: action.value }
    case 'SET_PREFERRED_NAME':
      return { ...state, preferredName: action.value }
    case 'SET_BIRTH_MONTH':
      return { ...state, birthMonth: action.value }
    case 'SET_BIRTH_YEAR':
      return { ...state, birthYear: action.value }
    case 'SET_SKIN_TYPE':
      return { ...state, skinType: action.value }
    case 'SET_PRIMARY_CONCERNS':
      return { ...state, primaryConcerns: action.value }
    case 'SET_GOALS':
      return { ...state, goals: action.value }
    case 'SET_EXPERIENCE':
      return { ...state, skincareExperience: action.value }
    case 'SET_SKIN_TONE':
      return { ...state, skinToneScale: action.value }
    case 'SET_SUN_RESPONSE':
      return { ...state, sunResponse: action.value }
    case 'SET_UNDERTONE':
      return { ...state, skinUndertone: action.value }
    case 'SET_PIH_FREQUENCY':
      return { ...state, pihFrequency: action.value }
    case 'SET_PIH_DURATION':
      return { ...state, pihDuration: action.value }
    case 'SET_UNEVEN_PATCHES':
      return { ...state, unevenPatches: action.value }
    case 'SET_PRODUCT_REACTIVITY':
      return { ...state, productReactivity: action.value }
    case 'SET_INFLAMMATORY_HISTORY':
      return { ...state, inflammatoryHistory: action.value }
    case 'SET_PRODUCT_REACTION_SEVERITY':
      return { ...state, productReactionSeverity: action.value }
    case 'SET_RECOVERY_TIME':
      return { ...state, recoveryTime: action.value }
    case 'SET_OILY_AND_TIGHT':
      return { ...state, oilyAndTight: action.value }
    case 'SET_BREAKOUT_PATTERN':
      return { ...state, breakoutPattern: action.value }
    case 'SET_BREAKOUT_AREAS':
      return { ...state, breakoutAreas: action.value }
    case 'SET_REDNESS_PATTERN':
      return { ...state, rednessPattern: action.value }
    case 'SET_REDNESS_AREAS':
      return { ...state, rednessAreas: action.value }
    case 'SET_FLUSH_TRIGGERS':
      return { ...state, flushTriggers: action.value }
    case 'SET_FLUSH_FADE_SPEED':
      return { ...state, flushFadeSpeed: action.value }
    case 'SET_CURRENT_STATE_NORMAL':
      return { ...state, currentStateNormal: action.value }
    case 'SET_CURRENT_STATE_DIFFS':
      return { ...state, currentStateDiffs: action.value }
    case 'SET_RECENT_CHANGE_ANSWER':
      return { ...state, recentChangeAnswer: action.value }
    case 'SET_RECENT_CHANGE_DETAIL':
      return { ...state, recentChangeDetail: action.value }
    case 'SET_DARKER_AREAS':
      return { ...state, darkerAreas: action.value }
    case 'SET_DARKER_AREA_TRIGGERS':
      return { ...state, darkerAreaTriggers: action.value }
    ...
    case 'SET_HOME_DEVICES':
      return { ...state, homeDevices: action.value }
    case 'SET_PRO_TREATMENTS':
      return { ...state, professionalTreatments: action.value }
    default:
      return state
  }
}
...
export function OnboardingWizard({
  ...
  // step 1.
  const resumeStep =
    initialStep <= 0 ? 0 : initialStep === 1 ? 1 : Math.min(initialStep + 1, TOTAL_STEPS)
  ...
  const [state, dispatch] = useReducer(wizardReducer, {
    ...
    recentChangeAnswer:
      initialProfile?.recentChange == null ? '' : initialProfile.recentChange ? 'yes' : 'no',
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          zIndex: 0,
          backgroundImage: `
            radial-gradient(circle at 10% 90%, rgba(196, 176, 154,0.05) 0%, transparent 50%),
            radial-gradient(circle at 90% 10%, rgba(196, 176, 154,0.04) 0%, transparent 45%)
          `,
        }}
      />
      ...
      <header
        style={{
          ...
          alignItems: 'center',
          gap: '2rem',
          borderBottom: '1px solid rgba(196, 176, 154,0.1)',
        }}
      >
        <span
          style={{
            ...
            <>
              ...
              {state.step === 11 && (
                <StepProductReactivity
                  ...
                  onSeverityChange={(v) =>
                    dispatch({ type: 'SET_PRODUCT_REACTION_SEVERITY', value: v })
                  }
                  onContinue={goNext}
                  onBack={goBack}
                />
              ...
              {state.step === 16 && (
                <StepRedness
                  ...
                  onFlushFadeSpeedChange={(v) =>
                    dispatch({ type: 'SET_FLUSH_FADE_SPEED', value: v })
                  }
                  onContinue={goNext}
                  onBack={goBack}
                />
              ...
              {state.step === 21 && (
                <StepTools
                  ...
                  onProfessionalTreatmentsChange={(v) =>
                    dispatch({ type: 'SET_PRO_TREATMENTS', value: v })
                  }
                  onContinue={goNext}
                  onBack={goBack}
                />
              ...
              {state.step === 22 && <StepInterpretation onContinue={goNext} onBack={goBack} />}

              {state.step === 23 && <StepCompletion onContinue={goNext} onBack={goBack} />}

              {state.step === 24 && <StepDossierIntro onBack={goBack} />}
            </>
          )}
        </div>
      </main>
