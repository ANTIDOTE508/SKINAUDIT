...
import type { RednessPattern } from '@prisma/client'
...
export function StepRedness({
  ...
  const showAreas = pattern != null && AREA_FOLLOWUP_PATTERNS.includes(pattern)
  ...
  const handlePatternChange = (v: RednessPattern) => {
    onPatternChange(v)
    // Leaving a "Yes" answer makes the area list irrelevant — clear it so a
    // stale selection isn't persisted.
    if (!AREA_FOLLOWUP_PATTERNS.includes(v) && areas.length > 0) onAreasChange([])
  }
